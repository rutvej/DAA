...
try:
    ...
    sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "../app/backend-api")))
    ...
    backend_app.dependency_overrides[get_current_user] = lambda: {"username": "demo-sre", "id": "demo-user-id"}
    ...
    print(f"[Note] Could not initialize standalone mode ({e}). Switching to Live Server mode...")
    import requests
...
def run_showcase():
    ...
    print(f"Mode: {'Standalone In-Memory Showcase' if STANDALONE_MODE else f'Live Server ({BASE_URL})'}")

    # --- STEP 1: Register Applications & Policies ---
    print_step(1, "Registering Microservices & SLA Escalation Policies...")
    
    status, app_res = api_post("/applications/", {
    ...
    status, policy_res = api_post(f"/applications/{app_id}/escalation-policies", {
    ...
    for i in range(1, 6):
        ...
        if "Logged (Threshold not reached)" in log_status:
            print(f"  ⚡ [Log {i}/5] Received Error -> Status: Logged ({log_res.get('error_count')}/3 in 60s window)")
        elif "Escalated to Agent" in log_status:
            inc_id = log_res.get("incidentId", "N/A")
            print(f"  🚨 [Log {i}/5] THRESHOLD BREACHED (3/3)! -> Status: Escalated to Agent!")
            ...
            print(f"  🛡️  [Log {i}/5] IDENTICAL ERROR DETECTED -> Status: Suppressed (Deduplicated)!")
            print(f"     ➔ SHA256 Fingerprint matched active incident. 0% Redundant LLM Token Waste!")
    ...
    if inc_list and len(inc_list) > 0:
        ...
        print(f"  ✔ Occurrence Count  : {inc.get('occurrence_count')} errors aggregated")
    ...
    print("    ➔ Found commit a8f92b: 'Update redis pool max_connections from 500 to 10'")
    time.sleep(0.5)
    ...
    time.sleep(0.5)
    print("  [Dimension 3: Correlated Traces] Executing `query_correlated_logs(trace_id)`...")
    print(f"    ➔ Correlated 3 failed requests across payment-service and checkout-service")
    time.sleep(0.5)
    print("  [Dimension 4: Surgical Code Nav] Executing `read_repomap` & `view_file_slice`...")
    print("    ➔ Read repomap (120 lines total). Located symbol `redis_pool_init` in config.py")
    print("    ➔ Viewed slice config.py:L15-L25 (10 lines read. NOT reading entire 5000-line repo!)")
