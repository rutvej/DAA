...
def test_v2_applications_and_policies():
    ...
    policy_res = client.post(f"/applications/{app_id}/escalation-policies", json={
...
def test_v2_deduplication_and_escalation(mock_pika):
    ...
    app_id = app_res.json()["id"]
    client.post(f"/applications/{app_id}/escalation-policies", json={
        "condition_value": 2,
        "window_seconds": 120
    })

    # 2. Submit 1st log -> Should be Logged (1/2 in 120s)
    log1 = client.post("/logs/", json={
