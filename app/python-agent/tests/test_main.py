...
class TestMain(unittest.TestCase):
    ...
    def test_process_job_daa20_fallback(self, mock_analysis_updater, mock_get_llm, mock_agent_executor, mock_run_preflight):
        ...
        self.assertEqual(called_args[0]["input"], f"Investigate across all 4 dimensions and remediate the outage in test-app. Error: {scrubbed_log}")
    ...
    def test_process_job_daa30(self, mock_analysis_updater, mock_get_llm, mock_safety_wrapper, mock_run_preflight, mock_postflight, mock_repo_cache):
        ...
        mock_safety_wrapper.return_value.invoke.return_value = {"output": "WRITE_DIFF:\n--- a/file\n+++ b/file\nEXPLANATION: Fix"}
        mock_updater_instance = mock_analysis_updater.return_value
        ...
        self.assertEqual(called_args[0]["input"], "[INCIDENT]\napp: test-app\nfingerprint: mock_fingerprint\n")
        mock_updater_instance.set_pull_request_url.assert_called_once_with('http://github.com/pr/1')
        mock_updater_instance.set_postmortem.assert_called_once_with('Some report')
        mock_updater_instance.update_analysis_completed.assert_called_once()
