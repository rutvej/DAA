...
def handle_request_with_retry(method: str, url: str, **kwargs) -> requests.Response:
    ...
    if res.status_code == 401:
        ...
        try:
            ...
            login_res = requests.post(login_url, json={"username": "testuser", "password": "testpassword"}, timeout=5)
            if login_res.status_code == 200:
                new_token = login_res.json().get("token") or login_res.json().get("access_token")
                if new_token:
