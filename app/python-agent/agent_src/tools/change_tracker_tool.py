...
class CheckRecentChangesInput(BaseModel):
    data: str = Field(
        description="A JSON string containing optional 'repo_path' (default '.') and optional 'hours' (default 24). Example: {\"repo_path\": \"/app\", \"hours\": 24}"
    )
...
def check_recent_changes(data: str) -> str:
    ...
    try:
        ...
        result = subprocess.run(
            cmd, cwd=full_dir, capture_output=True, text=True, check=False
        )
