...
class SearchRepoInput(BaseModel):
    query: str = Field(
        description="The search query to find relevant code snippets (e.g. 'redis cache session key timeout')"
    )
    repo_path: str = Field(
        default=".", description="The path to the cloned repository to search."
    )
    ...
    if os.environ.get("DAA_GIT_MODE") == "api":
        ...
        for match in results[:3]:
            ...
            if content:
                ...
                snippets.append(
                    f"=== File: {file_path} (Lines 1-{len(lines)}) ===\n"
                    + "\n".join(lines)
                )
        if snippets:
            return "\n\n".join(snippets)
