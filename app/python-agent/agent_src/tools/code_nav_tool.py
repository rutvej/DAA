...
class GrepSearchInput(BaseModel):
    data: str = Field(description="A JSON string containing 'query' (string to search) and optional 'search_path' (default '.'). Example: {\"query\": \"def process_payment\", \"search_path\": \"/app\"}")
...
def grep_search(data: str) -> str:
    ...
    try:
        ...
        for root, dirs, files in os.walk(full_dir):
            ...
            for file in files:
                if file.endswith((".pyc", ".o", ".exe", ".dll", ".so", ".png", ".jpg", ".zip")):
                    continue
...
class FindSymbolInput(BaseModel):
    data: str = Field(description="A JSON string containing 'symbol_name' (class, function, or struct name) and optional 'search_path' (default '.'). Example: {\"symbol_name\": \"PaymentService\", \"search_path\": \"/app\"}")
...
class ReadRepomapInput(BaseModel):
    data: str = Field(description="A JSON string containing optional 'repo_path' (default '.'). Example: {\"repo_path\": \"/app\"}")
...
def read_repomap(data: str) -> str:
    ...
    try:
        ...
        ignore_dirs = {".git", "node_modules", "__pycache__", ".venv", "venv", "bin", "obj", "dist", "build", "tests"}
        skeleton_lines = []
        max_lines = 1000
                    ...
                    if file.endswith(".py"):
                        ...
                        for node in tree.body:
                            if isinstance(node, ast.ClassDef):
                                ...
                                if doc:
                                    skeleton_lines.append(f"    \"\"\"{doc.splitlines()[0]}\"\"\"")
                                for item in node.body:
                                    if isinstance(item, (ast.FunctionDef, ast.AsyncFunctionDef)):
                                        skeleton_lines.append(f"    def {item.name}(...): ...")
                        ...
                        with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
                            for line in f:
                                stripped = line.strip()
                                if re.match(r"^(public|private|protected|export|default|func|class|interface|type|def)\s+", stripped):
