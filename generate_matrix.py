...
combinations = list(
    itertools.product(stagings, dbs, queues, gits, auths, policies, integrations)
)
...
with open("matrix.md", "w") as f:
    ...
    for i, c in enumerate(combinations):
        ...
        if staging == "Image" and git == "local":
            outcome = "❌ Fails"
            reason = (
                "Cloud Run (Image) lacks Docker socket required for 'local' testing."
            )
        ...
        f.write(
            f"| {i + 1} | {staging} | {db} | {queue} | {git} | {auth} | {policy} | {integration} | {outcome} | {reason} |\n"
        )
