...
- [ ] **Task 2 (`[P0-SEC-2]`): Restrict Overly Permissive LAN CORS Subnet Regex & Dynamic Origin Injection**
...
- [ ] **Task 3 (`[P0-SEC-3]`): Eliminate Synthetic `admin-id` Privilege Escalation Bypasses**
...
- [ ] **Task 4 (`[P0-SEC-4]`): Replace `shell=True` Command Injection Vulnerabilities with Safe Tokenized Arrays**
...
- [ ] **Task 6 (`[P0-OPS-2]`): Standardize RabbitMQ Queue Names (`fix_jobs` vs `DAA_RABBITMQ_QUEUE`)**
...
- [ ] **Task 7 (`[P0-OPS-3]`): Remove `MockSession` Silent Database Drop Mode & Enforce Fail-Fast Persistence**
...
- [ ] **Task 8 (`[P0-OPS-4]`): Fix MCP Client JSON-RPC Protocol Violation (`initialize` vs `tools/list`)**
...
- [ ] **Task 10 (`[P1-ARCH-2]`): Consolidate Duplicated SHA-256 Fingerprinting & Ast-Deduplication Engines**
...
- [ ] **Task 11 (`[P1-RES-1]`): Add Exponential Backoff & Circuit Breakers to LLM & Git API Providers**
...
- [ ] **Task 12 (`[P1-SEC-5]`): Restrict Arbitrary Host Path Traversal in `file_system_tool.py` & `code_nav_tool.py`**
